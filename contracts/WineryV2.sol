...
interface IWineryProgression {
    ...
    function getMafiaModifier(address _owner) external view returns (uint256);
}
...
contract Winery is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function _claimVintageWine(address _owner) internal {
        ...
        if (mafia.mafiaIsActive()) {
            uint256 mafiaSkillModifier = wineryProgression.getMafiaModifier(
                _owner
            );
            ...
            if (penalty < mafiaSkillModifier) {
                ...
                taxAmountMafia =
                    (totalClaimed * (penalty - mafiaSkillModifier)) /
                    100;
            }
        }
