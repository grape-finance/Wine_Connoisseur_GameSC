...
contract MafiaBoss is Ownable {
    ...
    function updateBribeMultiplier(uint256 _newbribeMultiplier)
        external
        onlyOwner
    {
    ...
    function bribeMafioso(uint256 _nrBribes) public {
        ...
        Mafioso storage mafioso = mafiosos[currentMafiosoId];
        uint256 _amountVintageWine = _nrBribes * mafioso.bribeAmount;
        ...
        uint256 bribeWithMultiplier = _amountVintageWine *
            mafioso.bribeMultiplier;
        ...
        if (playerIndex[currentMafiosoId][owner] == 0) {
            ...
            playerIndex[currentMafiosoId][owner] = numberOfPlayers[
                currentMafiosoId
            ];
            Player storage player = participantPlayers[currentMafiosoId][
                numberOfPlayers[currentMafiosoId]
            ];
            player.owner = owner;
            ...
            Player storage player = participantPlayers[currentMafiosoId][
                playerIndex[currentMafiosoId][owner]
            ];
            player.bribes += _nrBribes * mafioso.bribeMultiplier;
        }
    ...
    function getPlayerBribe(address _player) public view returns (uint256) {
        Player memory player = participantPlayers[currentMafiosoId][
            playerIndex[currentMafiosoId][_player]
        ];
