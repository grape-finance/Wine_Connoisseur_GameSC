...
interface IOldWineryProgression {
...
contract WineryProgression is Ownable {
    ...
    function fixSkillPoints(address _player) public {
        ...
        uint256 totalSkillsLearned = skillsLearned[_player][BURN_ID] +
            ...
            skillsLearned[_player][MAFIA_ID];
    ...
    // WINERY MIGRATION
    IOldWineryProgression public oldWineryProgression;
    ...
    function checkIfNeedUpdate(address _owner) public view returns (bool) {
        ...
        uint256 oldGrapeDeposited = oldWineryProgression.grapeDeposited(_owner);
    ...
    function setOldWineryProgression(address _oldWineryProgression) external onlyOwner {
        oldWineryProgression = IOldWineryProgression(_oldWineryProgression);
    }
    ...
    function updateDataFromOldWinery(address _owner) external {
        ...
        grapeDeposited[_owner] = oldWineryProgression.grapeDeposited(_owner);

        skillPoints[_owner] = oldWineryProgression.skillPoints(_owner);

        uint256 burnSkillId = oldWineryProgression.skillsLearned(_owner, 0);
        uint256 fatigueSkillId = oldWineryProgression.skillsLearned(_owner, 1);
        uint256 cellarSkillId = oldWineryProgression.skillsLearned(_owner, 2);
        uint256 mastervintnerSkillId = oldWineryProgression.skillsLearned(_owner, 3);
        uint256 upgradeSkillId = oldWineryProgression.skillsLearned(_owner, 4);
        uint256 vintnerSkillId = oldWineryProgression.skillsLearned(_owner, 5);
        uint256 storageSkillId = oldWineryProgression.skillsLearned(_owner, 6);
